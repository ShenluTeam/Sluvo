...
Canvas Agent controls live inside `CanvasWorkspaceView.vue` for the MVP. The right-side 创作总监 panel now starts Agent Team workflow runs, sends selected-node context, renders a multi-Agent message stream above the staged step/artifact timeline, confirms media cost, and keeps the legacy session/action proposal path compatible through `src/api/sluvoApi.js`.
