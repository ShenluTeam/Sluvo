...
- When an Agent workflow is waiting for confirmation, text entered in the composer is treated as feedback for the current stage; the explicit `继续下一步` button advances to the next Agent stage, and an exact typed confirmation phrase such as `满意，继续下一步` is handled as the same continue action instead of Review feedback.
- Agent confirmation controls should appear only after the current Agent has finished its visible thinking/result message. During Agent handoff, show the invitation first, pause briefly, then reveal the next Agent thinking state before its result.
- The Agent workflow overview in the right panel should always show the six production groups: `总览`, `剧本`, `角色/场景/道具`, `分镜`, `图片`, and `视频`; the current group is highlighted and completed groups are marked complete.
- The default Agent workflow order is: confirm inspiration/script plus duration/global settings, generate or revise script body, extract character/scene/prop assets and image placeholders, generate storyboard table, generate storyboard images, then generate shot videos and the merged video.
...
The right-side 创作总监 panel should work as an Agent Team control surface: users can select automatic routing, copy starter Agents into "我的 Agent", create/edit/delete custom Agents, restore project-local Run history, and inspect each Step/Artifact card with text labels for status, model, source context, canvas write state, retry, and cost confirmation. The default official team should follow the target creative pipeline: Onboarding, Scriptwriter, Visual Asset, Storyboard Artist, Image Generator, Video Generator, and Review.
