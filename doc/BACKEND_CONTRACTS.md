...
- Agent workflow specs expose `groupKey` and `groupLabel` for the six production groups: `overview`, `script`, `assets`, `storyboard`, `image`, and `video`. Onboarding context includes `durationMode` and `durationSeconds`; later image/video artifacts must read the same stored global media config.
- Agent artifact types include `script_body`, `character_brief`, `scene_brief`, `prop_brief`, `character_image_placeholder`, `scene_image_placeholder`, `prop_image_placeholder`, `storyboard_table`, `storyboard_image_placeholder`, `shot_video_placeholder`, and `merged_video_placeholder`.
