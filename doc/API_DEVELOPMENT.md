...
- OiiOii-style Agent Team workflows use `POST /api/sluvo/projects/{project_id}/agent/runs` and return a `Run -> Step -> Artifact` timeline. Text/story/character/scene/prompt artifacts and media placeholder nodes are written through `apply_sluvo_canvas_batch(... actor_type="agent")`; media artifacts remain in `waiting_cost_confirmation` until `POST /api/sluvo/agent/runs/{run_id}/confirm-cost` binds generation records.
