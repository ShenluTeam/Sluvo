...
- Staged Agent Team workflows use `POST /api/sluvo/projects/{project_id}/agent/runs` and return a `Run -> Step -> Artifact` timeline. The backend also stores openOii-style events such as `run_started`, `run_progress`, `run_message`, `agent_handoff`, `run_awaiting_confirm`, `run_confirmed`, and `run_completed` on the run's Agent session.
