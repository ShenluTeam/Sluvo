...
| Agent Run event stream | `GET /api/sluvo/agent/runs/{run_id}/events` |
...
- Staged Agent Team workflows use `POST /api/sluvo/projects/{project_id}/agent/runs` and return a `Run -> Step -> Artifact` timeline. The backend also stores multi-Agent conversation events such as `run_started`, `run_progress`, `run_message`, `agent_handoff`, `run_awaiting_confirm`, `run_confirmed`, and `run_completed` on the run's Agent session.
- `GET /api/sluvo/agent/runs/{run_id}/events` exposes an SSE stream for the current Run timeline. The browser passes `shenlu_token` as the `token` query parameter because native `EventSource` cannot set the `Authorization` header. Proxies must disable buffering for this path.
- Text/story/character/scene/prompt artifacts and media placeholder nodes are written through `apply_sluvo_canvas_batch(... actor_type="agent")`; media artifacts remain in `waiting_cost_confirmation` until `POST /api/sluvo/agent/runs/{run_id}/confirm-cost` binds generation records.
- `GET /api/sluvo/projects/{project_id}/agent/runs` restores recent project workflow runs after refresh, while `/api/sluvo/agent/runs/{run_id}/continue` appends follow-up steps and `/api/sluvo/agent/steps/{step_id}/retry` reruns failed steps without duplicating already-written successful artifacts.
